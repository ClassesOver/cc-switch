...
[Releases](../../releases) から Ubuntu 20.04 x64 向けの最新版をダウンロード：

- `CC-Switch-v{version}-Linux-Ubuntu20.04-glibc2.31-x64.deb`

> **互換性**：公式 Linux 配布は Ubuntu 20.04 x64 と `glibc 2.31` のみを対象にしています。

> **自動更新**：Linux の AppImage 成果物は内蔵アップデータ専用です。手動インストールは `.deb` を推奨します。
...
# Ubuntu 20.04 glibc 2.31 x64 の公式 Linux 配布物（.deb + AppImage）をビルド
pnpm build:linux:ubuntu20

# Ubuntu 20.04 glibc 2.31 x64 の .deb パッケージのみビルド
