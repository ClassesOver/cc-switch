...
1. Download `CC-Switch-v{version}-Linux-x86_64.deb` or `CC-Switch-v{version}-Linux-arm64.deb`
2. Install:

```bash
sudo dpkg -i CC-Switch-v{version}-Linux-<arch>.deb

# If there are dependency issues
sudo apt-get install -f
```

> The Linux AppImage artifact is reserved for the built-in auto-updater. Manual installation is recommended via `.deb`.
