...
1. 下载 `CC-Switch-v{版本号}-Linux-x86_64.deb` 或 `CC-Switch-v{版本号}-Linux-arm64.deb`
2. 安装：

```bash
sudo dpkg -i CC-Switch-v{版本号}-Linux-<arch>.deb

# 如果有依赖问题
sudo apt-get install -f
```

> Linux 的 AppImage 产物仅用于内置自动更新，手动安装请优先使用 `.deb`。
