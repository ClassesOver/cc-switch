...
1. `CC-Switch-v{バージョン}-Linux-x86_64.deb` または `CC-Switch-v{バージョン}-Linux-arm64.deb` をダウンロード
2. インストール：

```bash
sudo dpkg -i CC-Switch-v{バージョン}-Linux-<arch>.deb

# 依存関係に問題がある場合
sudo apt-get install -f
```

> Linux の AppImage 成果物は内蔵アップデータ専用です。手動インストールは `.deb` を推奨します。
